...
5.  **Data Adapter Pattern:**
    ...
    - ✅ **Đã triển khai DateTime Adapter**: Tạo module `datetime_adapter.py` để chuẩn hóa Neo4j DateTime sang chuẩn ISO 8601 dễ sử dụng trong API response.
    - ⚠️ **Tạm thởi bỏ qua validation nghiêm ngặt**: Đã tạm thởi bỏ `response_model` trong FastAPI để có thể linh hoạt xử lý dữ liệu legacy.
    - **Bài học từ Recognition API**:
      - Dữ liệu trong Neo4j chứa các kỹ thuật lưu trữ không đồng nhất (enum, datetime, trường bắt buộc). Cần cân bằng giữa cách migrate dữ liệu hoặc sử dụng adapter.
      - FastAPI `response_model` rất nghiêm ngặt về validation, nên trong trường hợp legacy data có thể tạm thởi bỏ qua và xử lý thủ công.
    ...
    - **Kế hoạch nâng cao**:
      ...
      - Xây dựng các migration script để chuẩn hóa dữ liệu legacy, từ đó có thể bật `response_model` validation.
